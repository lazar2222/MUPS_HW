...
int main(int arc, char **argv)
{
  ...
  #pragma omp parallel for collapse(1) schedule(dynamic, 1) default(none) private(i,j,k,we,vh,dx,dy,dz,vs,us,ut,x,y,z,x1,x2,x3,chk,w_exact,wt,steps_ave,w,trial,steps) shared(ni,nj,nk,N,h,stepsz,a,b,c) firstprivate(seed) reduction(+:err,n_inside)
  for (i = 1; i <= ni; i++)
  {
    for (j = 1; j <= nj; j++)
    {

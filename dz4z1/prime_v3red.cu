...
__global__ void prime_number(int* globalTotal, int n)
{
  ...
  if (threadIdx.x == 0) {
    globalTotal[blockIdx.x]=total[0];
  }
}
...
void test(int n_lo, int n_hi, int n_factor)
{
  ...
  while (n <= n_hi)
  {
    ...
    hostTotal =(int*) malloc(sizeof(int)*gridDim.x);
    cudaMalloc(&globalTotal, sizeof(int)*gridDim.x);
    ...
    cudaMemcpy(hostTotal,globalTotal,sizeof(int)*gridDim.x,cudaMemcpyDeviceToHost);    
